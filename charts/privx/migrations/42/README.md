Upgrade Guide to PrivX 42.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 41.X to 42.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/42/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_41.X.X`
...
command to upgrade to PrivX 42.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/42/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_41.X.X`). Make
...
If your backup folder was named `privx-backup-PPPPP_YYYY-MM-DD-hhmm_41.X.X` then your `PREVIOUS_VERSION` will be `41.X.X`.

4. Restore PrivX settings using the following command:
```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/42/restore.yaml \
    privx -n privx charts/privx/
```
