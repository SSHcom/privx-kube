Upgrade Guide to Privx 25.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 24.X to 25.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/25/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_24.0.0`
...
command to upgrade to Privx 25.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/25/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_24.0.0`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/25/restore.yaml \
    privx -n privx charts/privx/
```
