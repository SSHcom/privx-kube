Upgrade Guide to Privx 28.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 27.X to 28.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/28/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_27.X.X`
...
command to upgrade to Privx 28.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/28/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_27.X.X`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/28/restore.yaml \
    privx -n privx charts/privx/
```
