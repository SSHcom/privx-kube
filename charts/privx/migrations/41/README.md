Upgrade Guide to PrivX 41.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 39.X to 41.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/41/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_39.X.X`
...
command to upgrade to PrivX 41.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/41/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_39.X.X`). Make
...
If your backup folder was named `privx-backup-PPPPP_YYYY-MM-DD-hhmm_39.X.X` then your `PREVIOUS_VERSION` will be `39.X.X`.

4. Restore PrivX settings using the following command:
```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/41/restore.yaml \
    privx -n privx charts/privx/
```
