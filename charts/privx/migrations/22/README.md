Upgrade Guide to Privx 22.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 21.X to 22.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/22/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_21.0.0`
...
command to upgrade to Privx 22.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/22/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_21.0.0`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/22/restore.yaml \
    privx -n privx charts/privx/
```
