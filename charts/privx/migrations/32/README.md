Upgrade Guide to Privx 32.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 31.X to 32.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/32/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_31.X.Y`
...
command to upgrade to Privx 32.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/32/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_31.X.Y`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/32/restore.yaml \
    privx -n privx charts/privx/
```
