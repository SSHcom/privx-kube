Upgrade Guide to Privx 23.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 22.X to 23.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/23/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_22.1.0`
...
command to upgrade to Privx 23.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/23/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_22.1.0`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/23/restore.yaml \
    privx -n privx charts/privx/
```
