Upgrade Guide to Privx 24.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 23.X to 24.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/24/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_23.X.X`
...
command to upgrade to Privx 24.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/24/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_23.X.X`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/24/restore.yaml \
    privx -n privx charts/privx/
```
