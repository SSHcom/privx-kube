Upgrade Guide to Privx 31.X
======================================

## Current Helm revision for PrivX

Before proceeding to upgrade PrivX from 30.X to 31.X, take a note of the current
...
helm upgrade --history-max 0 \
    ...
    -f charts/privx/migrations/31/stage1.yaml \
...
`privx-backup-PPPPP_YYYY-MM-DD-hhmm_30.X.X`
...
command to upgrade to Privx 31.X.

```
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/31/stage2.yaml \
...
when the backup was taken (`privx-backup-PPPPP_YYYY-MM-DD-hhmm_30.X.X`). Make
...
helm upgrade --history-max 0 \
    -f values-overrides/privx.yaml \
    -f charts/privx/migrations/31/restore.yaml \
    privx -n privx charts/privx/
```
